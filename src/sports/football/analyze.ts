// Football analysis orchestration. Moved out of features/match-detail so the
// generic useAnalysis hook can stay sport-agnostic and delegate here via the
// football SportModule. Behaviour is unchanged from the previous inline impl.
...
import { FOOTBALL_BUNDLE, runBondedAnalysis } from "@/engine";
import {
  computeSyntheticAH,
  computeSyntheticOU,
  type SyntheticPrice,
...
export const FOOTBALL_ODDS_SLUG = "football";
...
export const analyzeFootball = async (args: AnalyzeArgs): Promise<AnalysisResult> => {
  const { match, signal: parentSignal, forceRefresh = false } = args;
  ...
  const { oddsComponents, splits: splitProvider, history: historyProvider } =
    resolveProviders(settings, FOOTBALL_ODDS_SLUG);
  ...
  try {
    ...
    console.warn("[analyzeFootball] synthetic line generation failed", err);
  }
  ...
  const { candidates: allCandidates, combos, diagnostics } = runBondedAnalysis(
    ctx,
    { includePass: true },
    FOOTBALL_BUNDLE,
  );
